...
@app.route("/")
def root():
    return {"message": "Cyberpunk Terminal API is running!"}
...
@app.route("/file-content/<path:path>")
def get_file_content(path):
    try:
        ...
        return jsonify({"content": "Binary file - cannot display", "path": path})
    except Exception as e:
        return jsonify({"error": f"Error reading file: {str(e)}"}), 500

@app.route("/speech-to-text", methods=["POST"])
def speech_to_text():
    return jsonify({"text": "Voice command not implemented yet", "error": None})
